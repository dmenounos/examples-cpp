...
auto AlgorithmIterator::operator!=(const Iterator<int>& other) -> bool {
	LOG_D("AlgorithmIterator::operator!=(const Iterator& other)");
	this->advance(false);
	return _hasNext;
}
...
auto AlgorithmIterator::operator++() -> Iterator<int>& {
	LOG_D("AlgorithmIterator::operator++()");
	this->advance(true);
	return *this;
}
...
auto AlgorithmIterator::operator++(int) -> void {
